...
\date{{\bf AASS Seminar} | 15 Sep 2022}
...
{
    \paper{Newbutt et al. \textbf{Co-designing a social robot in a SEN school:
    Listening to the ambitions of autistic children and their teachers}
...
{
    \paper{Lemaignan et al. \textbf{A social robot in a school for
autistic children} IJSR 2022}
...
\begin{frame}{Impact on well-being}
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{center}
                \includegraphics[width=\linewidth]{figs/robots4sen/moods_plot.pdf}
            \end{center}
        \end{column}
    \end{columns}
...
\end{tabular}
    ...
    detection}. \only<2>{\tiny In {\bf bold}, non-task specific features.}
}
    \only<3->{
